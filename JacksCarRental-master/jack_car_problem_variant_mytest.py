...
def apply_action(state, action):
    new_s = [0,0]
    new_s[0] = state[0] - action - 1
    new_s[1] = state[1] + action + 1
        
    if (new_s[0] > 10):
        
    return [max(min(state[0] - action - 1, jcp.max_cars()),0) , max(min(state[1] + action + 1, jcp.max_cars()),0)]
